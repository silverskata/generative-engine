...
void read_keyboard_released(button_control_t * buttons){
    ...
    if(keys==-1){
        ...
        for(uint8_t i = 0; i< 13;i++){
            if((keys >> i) & 1){
                if (buttons->keyboard_buttons[0] ==i ) keyhits[0] = 1;
                else if (buttons->keyboard_buttons[1] ==i) keyhits[1] = 1;
                else if (buttons->keyboard_buttons[2] == i) keyhits[2] = 1;
            }
            }      
        for(uint8_t i = 0; i < 2;i++){
...
void read_control_buttons(button_control_t * buttons){
    if(queue_is_full(&buttons->controlqueue)) return;
    ...
    for(uint8_t i = 0; i< 13;i++)
        ...
        switch(keycount){
            case 2:
                    ...
                    if(keypresses[1]==buttons->keyboard_buttons[0]) buttons->keyboard_buttons[1] = keypresses[0];
                    else buttons->keyboard_buttons[1] = keypresses[1];

...
bool update_shift(controller_t *self, uint16_t button_press){
    if(button_press>>1 != BUTTON_SHIFT) return false;
    if (is_input(button_press)) self->shift = true;
    else self->shift = false;
    return true;
}
//0 for off and 1 for on
void set_keyboard(controller_t *self, uint8_t onoff){
    if(onoff!=0)self->keyboard_active =3;
    else self->keyboard_active = 1;
}
...
void update_state_playing(controller_t * self, uint16_t button_press, int16_t keyboard_press,int8_t encoder_direction){
    if(self->shift) set_tempo(self->sequencer, encoder_direction*5);
    else set_tempo(self->sequencer, encoder_direction);

    if(button_press!=0 && !is_input(button_press))
    switch(button_press>>1){
        case BUTTON_LEFT:
            if(!self->sequencer->playing) self->sequencer->current_step=0;
            break;
        case(BUTTON_RIGHT):
    ...
    if(button_press!=0 && !is_input(button_press)){
        switch(button_press>>1){
            case BUTTON_LEFT: 
                ...
                break;
            case(BUTTON_RIGHT):
                if(self->shift) sequencer_pause_play(self->sequencer);
                else{
            ...
            case(BUTTON_UP):
                ...
                if(self->sequencer->sequencers[self->sequencer->active_sequence].note_value[self->sequencer->sequencers
                    ...
                    self->note.octave ++;
                    if(self->note.octave >5) self->note.octave = 5;
                }
            break;
            case(BUTTON_DOWN):
                ...
                if(self->sequencer->sequencers[self->sequencer->active_sequence].note_value[self->sequencer->sequencers
                    ...
                    [self->sequencer->active_sequence].current_page][self->sequencer->sequencers[self->sequencer->active_sequence].selected_step].type!=REST_NOTE) {
                    self->note.octave --;
                    if(self->note.octave > 5)self->note.octave = 0;
                }
            break;
            case(BUTTON_OK):
        ...
        switch(keyboard_press){
            ...
            case 6:
            self->note.legato --;
            if(self->note.legato<1) self->note.legato=1;
            break;
            case 8:
            self->note.legato ++;
...
void update_state_main(controller_t * self, uint16_t button_press, int16_t keyboard_press, int8_t encoder_direction){
    ...
    if(button_press!=0 && !is_input(button_press)){
        switch(button_press>>1){
            ...
            break;
            case(BUTTON_OK):
                if(self->shift && self->selection_state == 0) self->menu_state = MAIN_PLAYING;
                else
    ...
    if(selection_direction!=0)
        switch(self->selection_state){
            ...
            break;
            case 6: // TEMPO
            if(self->shift) set_tempo(self->sequencer, selection_direction*5);
            else set_tempo(self->sequencer, selection_direction);
            break;
        }
}
...
void update_state_generation(controller_t * self, uint16_t button_press, int8_t encoder_direction){
    ...
    switch(button_press>>1){
        ...
        if(self->shift && self->selection_state == 0) self->menu_state = MAIN_PLAYING;
        else if(self->shift && self->selection_state != 0) self->selection_state = 0;
            break;
        case(BUTTON_RIGHT):
            break;
        case(BUTTON_OK):
